...
contract FlightSuretyData {
    ...
    mapping(address => mapping(address => bool)) private addressAlreadyVotedForAirline; // votesByAirline
    ...
    mapping(address => uint) public votesByAddress;
    ...
    /*                                       EVENT DEFINITIONS                                  */
    /********************************************************************************************/
    event PolicyHolderPayoutReceived(address policyHolder, uint amount);
    ...
    */
    ...
    {
        require(airlineIsRegistered(airlineAddress), "Airline is not registered yet");
        _;  // All modifiers require an "_" which indicates where the function body will be added
    }
    ...
    */
    ...
    {
        require(false==airlineExists(airlineAddress), "Airline already exists");
        _;  // All modifiers require an "_" which indicates where the function body will be added
    }
    ...
    */      
    function getMaxInsurancePremiumValue() 
            public 
            view 
            returns(uint256) {
    ...
    * @return A bool that represents if the airline is registered
    */      
    function setRegistrationStatus(address newAirlineAddress, bool registrationStatus)
    ...
    function setAuthorizedCaller(address authorizedCaller)
        requireIsOperational
        requireContractOwner public{
    ...
    * @return 
    */      
    function incrementAirlineNumber()
            requireIsOperational
            public{
    ...
    */    
    ...
    external returns(uint256){
        bool alreadyVoted = addressAlreadyVotedForAirline[electorAirlineAddress][newAirlineAddress];

        if(alreadyVoted!=true){
            airlines[newAirlineAddress].numberOfVotes = 1 + airlines[newAirlineAddress].numberOfVotes;
        }

        addressAlreadyVotedForAirline[electorAirlineAddress][newAirlineAddress] = true;
    ...
    */   
    function buy(address airlineAddress, 
                ...
                payable 
                requireIsOperational {
        require(amount > 0, "Flight insurance is not free, premium payment needed (Data contract)");
        require(amount <= MAX_INSURANCE_PREMIUM_VALUE, "Flight premium amount can't exceed 1ETH (Data contract)");

        bytes32 flightKey = getFlightKey(airlineAddress, flight, timestamp);
        premiums[passengerAddress][flightKey] = premiums[passengerAddress][flightKey].add(amount);
    ...
    */
    function creditInsurees(bytes32 flightKey)
		...
		for (uint i = 0; i < policyHolders[flightKey].length; i++) {
            address passengerAddress = policyHolders[flightKey][i];
			uint256 reimburseAmount = premiums[passengerAddress][flightKey].mul(REIMBURSE_RATE).div(100);
     ...
     *  @dev Credits payouts to insurees
    */
//    function insureeBalance(address policyHolderAddress, flight)
//                            external
//                            requireIsOperational
//                            returns (uint256) {
//
//        return premiums[passengerAddress][flightKey];
//    }
     ...
     *
    */
    function pay(address policyHolderAddress)
             external 
             requireIsOperational {
        require(payouts[policyHolderAddress]>0,   "No payouts available");
        require(policyHolderAddress == tx.origin, "Payouts to contracts forbidden");
    ...
    */   
    function fund(address airlineAddress, uint256 amount) 
        ...
        uint256 alreadyFundedAmount = airlines[airlineAddress].airlineStackedEth;
        airlines[airlineAddress].airlineStackedEth = alreadyFundedAmount.add(amount);
